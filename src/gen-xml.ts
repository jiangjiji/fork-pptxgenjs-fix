...
import { convertRotationDegrees, createColorElement, createGlowElement, createTextShadow, encodeXmlEntities, genXmlColorSelection, getSmartParseNumber, getUuid, inch2Emu, valToPts } from './gen-utils'
...
function slideObjectToXml(slide: PresSlide | SlideLayout): string {
	...
	} else if (slide.background?.color || slide.background?.gradColor) {
		...
		switch (slideItemObj._type) {
				...
				*/
				...
				arrTabRows.forEach((cells, rIdx) => {
					...
					cells.forEach((cellObj) => {
						...
						let fillColor = cell._optImp?.fill?.color ? cell._optImp.fill.color : cell._optImp?.fill && typeof cell._optImp.fill === 'string' ? cell._optImp.fill : ''
			...
			case SLIDE_OBJECT_TYPES.placeholder:
				...
				if (slideItemObj.options.shadow && slideItemObj.options.shadow.type !== 'none') {
					...
					strSlideXml += ` <a:${slideItemObj.options.shadow.type}Shdw ${slideItemObj.options.shadow.type === 'outer' ? 'sx="100000" sy="100000" kx="0" ky="0" algn="bl" rotWithShape="0"' : ''} blurRad="${slideItemObj.options.shadow.blur}" dist="${slideItemObj.options.shadow.offset}" dir="${
			...
			case SLIDE_OBJECT_TYPES.image:
				...
				if (slideItemObj.options.shadow && slideItemObj.options.shadow.type !== 'none') {
					...
					slideItemObj.options.shadow.offset = valToPts(slideItemObj.options.shadow.offset ?? 0)
					slideItemObj.options.shadow.angle = Math.round((slideItemObj.options.shadow.angle ?? 0) * 60000)
					slideItemObj.options.shadow.opacity = Math.round((slideItemObj.options.shadow.opacity ?? 0) * 100000)
					slideItemObj.options.shadow.color = slideItemObj.options.shadow.color || DEF_TEXT_SHADOW.color
...
function genXmlTextRunProperties(opts: ObjectOptions | TextPropsOptions, isDefault: boolean): string {
	...
	if (opts.hyperlink) {
		if (typeof opts.hyperlink !== 'object') {
			...
			runProps += `<a:hlinkClick r:id="rId${opts.hyperlink._rId}" invalidUrl="" action="" tgtFrame="" tooltip="${opts.hyperlink.tooltip ? encodeXmlEntities(opts.hyperlink.tooltip) : ''}" history="1" highlightClick="0" endSnd="0"${opts.fontColor ? '>' : '/>'}`
		} else if (opts.hyperlink.slide) {
			runProps += `<a:hlinkClick r:id="rId${opts.hyperlink._rId}" action="ppaction://hlinksldjump" tooltip="${opts.hyperlink.tooltip ? encodeXmlEntities(opts.hyperlink.tooltip) : ''}"${opts.fontColor ? '>' : '/>'}`
		}
		if (opts.fontColor) {
			runProps += ' <a:extLst>'
...
export function makeXmlPresentation(pres: IPresentationProps): string {
	...
	// STEP 7: Add fonts
	interface FontFamliy {
		fontName: string

		fontRegular?: FontInfo
		fontBold?: FontInfo
		fontItalic?: FontInfo
		fontBoldItalic?: FontInfo
	}

	const mergeFontFamliy = (): FontFamliy[] => {
		const mergeFontList = []

		pres.fonts.forEach((font: FontInfo) => {
			const fontName = font.typeface
			let mergeFont = mergeFontList.find((item: FontFamliy) => item.fontName === fontName)
			if (!mergeFont) {
	...
	if (hasEmbedFonts) {
		...
		const mergeFontList = mergeFontFamliy()

		mergeFontList.forEach((fontFamliy: FontFamliy) => {
			// fontdata
