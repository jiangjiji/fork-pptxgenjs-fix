		...
		switch (slideItemObj._type) {
			...
			case SLIDE_OBJECT_TYPES.placeholder:
				...
				if (
					...
					slideItemObj.options.shadow.blur = valToPts(
						slideItemObj.options.shadow.blur || 0,
					)
					slideItemObj.options.shadow.offset = valToPts(
						slideItemObj.options.shadow.offset || 0,
					)
			...
			case SLIDE_OBJECT_TYPES.image:
				...
				if (
					...
					slideItemObj.options.shadow.opacity = Math.round(
						(slideItemObj.options.shadow.opacity ?? 0) * 100000,
					)
					slideItemObj.options.shadow.color =
						slideItemObj.options.shadow.color || DEF_TEXT_SHADOW.color
...
export function makeXmlPresentationRels (
	slides: PresSlide[],
	fonts: FontInfo[],
	updateFonts: (nFonts: FontInfo[]) => void,
): string {
	let intRelNum = 1
	let strXml = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + CRLF
	strXml +=
...
export function makeXmlPresentation (pres: IPresentationProps): string {
	...
	const mergeFontFamliy = (): FontFamliy[] => {
		...
		pres.fonts.forEach((font: FontInfo) => {
			...
			let mergeFont = mergeFontList.find(
				(item: FontFamliy) => item.fontName === fontName,
			)
			if (!mergeFont) {
